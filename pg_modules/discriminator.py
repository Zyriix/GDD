...
class SingleDisc(nn.Module):
    def __init__(self, nc=None, ndf=None, out_dim=1, start_sz=256, end_sz=8, head=None, patch=False,**kwargs):
        super().__init__()
        # midas channels
        ...
        layers.append(conv2d(nfc[end_sz], out_dim, kernel, 1, 0, bias=False))
...
class SingleDiscCond(nn.Module):
    def __init__(self, nc=None, ndf=None, start_sz=256, end_sz=8, head=None, patch=False, c_dim=1000, cmap_dim=64, rand_embedding=False,**kwargs):
        super().__init__()
        ...
        dist.print0(f'loaded imagenet embeddings from {embed_path}: {self.embed}')
        if rand_embedding:
            self.embed.__init__(num_embeddings=self.embed.num_embeddings, embedding_dim=self.embed.embedding_dim)
            dist.print0(f'initialized embeddings with random weights')
...
class MultiScaleD(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        dist.print0("Conditional Disc",cond)

        mini_discs = []
        end_sz = 8 if self.disc_in_res[-1] >= 8 else  self.disc_in_res[-1]
        for i, (cin, res) in enumerate(zip(self.disc_in_channels, self.disc_in_res)):
            start_sz = res if not patch else 16
            mini_discs += [str(i), Disc(nc=cin, start_sz=start_sz, end_sz=end_sz, patch=patch,rand_embedding=rand_embedding)],
...
class ProjectedDiscriminatorOnly(torch.nn.Module):
    def __init__(
        self,
        backbones,
        channels,  
    ...
    def forward(self, features,c):
        logits = []
        for feat, disc in zip(features,self.discriminators.values()):
            logits += disc(feat, c)

        return logits


class ProjectedFeatureOnly(torch.nn.Module):
    def __init__(
        self,
        backbones,
        diffaug=True,
