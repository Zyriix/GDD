...
class AugmentPipe(torch.nn.Module):
    ...
    def forward(self, images, images2=None, debug_percentile=None):
        ...
        if self.xint > 0:
            ...
            labels += [torch.round(t[:,0] * width).div(width * self.xint_max),torch.round(t[:,1] * height).div(height* self.xint_max)]
        ...
        if G_inv is not I_3:
            ...
            if images2 is not None:
                images2 = torch.nn.functional.pad(input=images2, pad=[mx0,mx1,my0,my1], mode='reflect')
            ...
            if images2 is not None:
                images2 = upfirdn2d.upsample2d(x=images2, f=self.Hz_geom, up=2)
            ...
            if images2 is not None:
                images2 = grid_sample_gradfix.grid_sample(images2, grid)
            # Downsample and crop.
            images = upfirdn2d.downsample2d(x=images, f=self.Hz_geom, down=2, padding=-Hz_pad*2, flip_filter=True)
            if images2 is not None:
                images2 = upfirdn2d.downsample2d(x=images2, f=self.Hz_geom, down=2, padding=-Hz_pad*2, flip_filter=True)
        # --------------------------------------------
        ...
        if C is not I_4:
            ...
            if images2 is not None:
                images2 = images2.reshape([batch_size, num_channels, height * width])
                
                if num_channels == 3:
                    images2 = C[:, :3, :3] @ images2 + C[:, :3, 3:]
                elif num_channels == 1:
                    C = C[:, :3, :].mean(dim=1, keepdims=True)
                    images2 = images2 * C[:, :, :3].sum(dim=2, keepdims=True) + C[:, :, 3:]
                else:
                    raise ValueError('Image must be RGB (3 channels) or L (1 channel)')
                images2 = images2.reshape([batch_size, num_channels, height, width])
        # ----------------------
        ...
        if self.noise > 0:
            ...
            sigma_z = torch.randn([batch_size, num_channels, height, width], device=device) * sigma
            images = images + sigma_z
            if images2 is not None:
                images2 = images2 + sigma_z
        ...
        if self.cutout > 0:
            ...
            labels += [size]
        labels = torch.cat([x.to(torch.float32).reshape(batch_size, -1) for x in labels], dim=1)
        # print(labels.shape)
        if images2 is not None:
